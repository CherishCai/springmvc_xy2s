...
public class BbsController extends BaseController {
	...
	// 准备添加数据
	@RequestMapping("createBbs.shtml")
	...
	public String addBbs(Bbs bbs) {
		...
		return "redirect:/bbs/createBbs.shtml";
	}

	// 通过主键删除数据
	@RequestMapping("deleteBbs.shtml")
	public String deleteBbs(String id) {
		this.bbsDAO.deleteBbs(id);
		return "redirect:/bbs/getAllBbs.shtml";
	}

	// 更新数据
	@RequestMapping("updateBbs.shtml")
	...
	// 显示全部数据
	@RequestMapping("getAllBbs.shtml")
	public String getAllBbs(String number, Map<String, Object> map) {
		...
		if ((Integer.parseInt(number) + 1) == 1) {
			...
			buffer.append("<a href=\"bbs/getAllBbs.shtml?number=0\">首页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if ((Integer.parseInt(number) + 1) == 1) {
			buffer.append("上一页");
		} else {
			buffer.append("<a href=\"bbs/getAllBbs.shtml?number=" + (Integer.parseInt(number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("下一页");
		} else {
			buffer.append("<a href=\"bbs/getAllBbs.shtml?number=" + (Integer.parseInt(number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("尾页");
		} else {
			buffer.append("<a href=\"bbs/getAllBbs.shtml?number=" + (maxPage - 1) + "\">尾页</a>");
		}
	...
	// 按条件查询数据 (模糊查询)
	@RequestMapping("queryBbsByCond.shtml")
	...
	// 按主键查询数据
	@RequestMapping("getBbsById.shtml")
