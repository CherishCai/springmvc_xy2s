...
public class ArticleController {
	...
	// 准备添加数据
	@RequestMapping("createArticle.shtml")
	...
	public String addArticle(Article article) {
		article.setArticleid(VeDate.getStringDatex());
		article.setAddtime(VeDate.getStringDateShort());
		article.setHits("0");
		this.articleDAO.insertArticle(article);
		return "redirect:/article/createArticle.shtml";
	}

	// 通过主键删除数据
	@RequestMapping("deleteArticle.shtml")
	public String deleteArticle(String id) {
		this.articleDAO.deleteArticle(id);
		return "redirect:/article/getAllArticle.shtml";
	}

	// 更新数据
	@RequestMapping("updateArticle.shtml")
	...
	// 显示全部数据
	@RequestMapping("getAllArticle.shtml")
	public String getAllArticle(String number, Map<String, Object> map) {
		...
		if ((Integer.parseInt(number) + 1) == 1) {
			...
			buffer.append("<a href=\"article/getAllArticle.shtml?number=0\">首页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if ((Integer.parseInt(number) + 1) == 1) {
			buffer.append("上一页");
			...
			buffer.append(
					"<a href=\"article/getAllArticle.shtml?number=" + (Integer.parseInt(number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			...
			buffer.append(
					"<a href=\"article/getAllArticle.shtml?number=" + (Integer.parseInt(number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("尾页");
		} else {
			buffer.append("<a href=\"article/getAllArticle.shtml?number=" + (maxPage - 1) + "\">尾页</a>");
		}
	...
	// 按条件查询数据 (模糊查询)
	@RequestMapping("queryArticleByCond.shtml")
	...
	// 按主键查询数据
	@RequestMapping("getArticleById.shtml")
