...
public class UsersController extends BaseController {
	...
	// 准备添加数据
	@RequestMapping("createUsers.shtml")
	public String createUsers(Map<String, Object> map) {
		return "admin/addusers";
	}

	// 添加数据
	@RequestMapping("addUsers.shtml")
	public String addUsers(Users users) {
		users.setUsersid(VeDate.getStringDatex());
		this.usersDAO.insertUsers(users);
		return "redirect:/users/createUsers.shtml";
	}

	// 通过主键删除数据
	@RequestMapping("deleteUsers.shtml")
	public String deleteUsers(String id) {
		this.usersDAO.deleteUsers(id);
		return "redirect:/users/getAllUsers.shtml";
	}

	// 更新数据
	@RequestMapping("updateUsers.shtml")
	...
	// 显示全部数据
	@RequestMapping("getAllUsers.shtml")
	public String getAllUsers(String number, Map<String, Object> map) {
		...
		if ((Integer.parseInt(number) + 1) == 1) {
			...
			buffer.append("<a href=\"users/getAllUsers.shtml?number=0\">首页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if ((Integer.parseInt(number) + 1) == 1) {
			buffer.append("上一页");
		} else {
			buffer.append("<a href=\"users/getAllUsers.shtml?number=" + (Integer.parseInt(number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("下一页");
		} else {
			buffer.append("<a href=\"users/getAllUsers.shtml?number=" + (Integer.parseInt(number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("尾页");
		} else {
			buffer.append("<a href=\"users/getAllUsers.shtml?number=" + (maxPage - 1) + "\">尾页</a>");
		}
	...
	// 按条件查询数据 (模糊查询)
	@RequestMapping("queryUsersByCond.shtml")
	...
	// 按主键查询数据
	@RequestMapping("getUsersById.shtml")
