...
public class TopicController extends BaseController {
	...
	// 准备添加数据
	@RequestMapping("createTopic.shtml")
	...
	public String addTopic(Topic topic) {
		...
		return "redirect:/topic/createTopic.shtml";
	}

	// 通过主键删除数据
	@RequestMapping("deleteTopic.shtml")
	public String deleteTopic(String id) {
		this.topicDAO.deleteTopic(id);
		return "redirect:/topic/getAllTopic.shtml";
	}

	// 更新数据
	@RequestMapping("updateTopic.shtml")
	...
	// 显示全部数据
	@RequestMapping("getAllTopic.shtml")
	public String getAllTopic(String number, Map<String, Object> map) {
		...
		if ((Integer.parseInt(number) + 1) == 1) {
			...
			buffer.append("<a href=\"topic/getAllTopic.shtml?number=0\">首页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if ((Integer.parseInt(number) + 1) == 1) {
			buffer.append("上一页");
		} else {
			buffer.append("<a href=\"topic/getAllTopic.shtml?number=" + (Integer.parseInt(number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("下一页");
		} else {
			buffer.append("<a href=\"topic/getAllTopic.shtml?number=" + (Integer.parseInt(number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("尾页");
		} else {
			buffer.append("<a href=\"topic/getAllTopic.shtml?number=" + (maxPage - 1) + "\">尾页</a>");
		}
	...
	// 按条件查询数据 (模糊查询)
	@RequestMapping("queryTopicByCond.shtml")
	...
	// 按主键查询数据
	@RequestMapping("getTopicById.shtml")
