...
public class CartController extends BaseController {
	...
	// 准备添加数据
	@RequestMapping("createCart.shtml")
	...
	public String addCart(Cart cart) {
		...
		return "redirect:/cart/createCart.shtml";
	}
	...
	public String deleteCart(String id) {
		...
		return "redirect:/cart/getAllCart.shtml";
	}

	// 更新数据
	@RequestMapping("updateCart.shtml")
	...
	// 显示全部数据
	@RequestMapping("getAllCart.shtml")
	public String getAllCart(String number, Map<String, Object> map) {
		...
		if ((Integer.parseInt(number) + 1) == 1) {
			...
			buffer.append("<a href=\"cart/getAllCart.shtml?number=0\">首页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if ((Integer.parseInt(number) + 1) == 1) {
			buffer.append("上一页");
		} else {
			buffer.append("<a href=\"cart/getAllCart.shtml?number=" + (Integer.parseInt(number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("下一页");
		} else {
			buffer.append("<a href=\"cart/getAllCart.shtml?number=" + (Integer.parseInt(number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("尾页");
		} else {
			buffer.append("<a href=\"cart/getAllCart.shtml?number=" + (maxPage - 1) + "\">尾页</a>");
		}
	...
	// 按条件查询数据 (模糊查询)
	@RequestMapping("queryCartByCond.shtml")
	...
	// 按主键查询数据
	@RequestMapping("getCartById.shtml")
