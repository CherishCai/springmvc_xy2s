...
public class FavController extends BaseController {
	...
	// 准备添加数据
	@RequestMapping("createFav.shtml")
	...
	public String addFav(Fav fav) {
		...
		return "redirect:/fav/createFav.shtml";
	}
	...
	public String deleteFav(String id) {
		...
		return "redirect:/fav/getAllFav.shtml";
	}

	// 更新数据
	@RequestMapping("updateFav.shtml")
	...
	// 显示全部数据
	@RequestMapping("getAllFav.shtml")
	public String getAllFav(String number, Map<String, Object> map) {
		...
		if ((Integer.parseInt(number) + 1) == 1) {
			...
			buffer.append("<a href=\"fav/getAllFav.shtml?number=0\">首页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if ((Integer.parseInt(number) + 1) == 1) {
			buffer.append("上一页");
		} else {
			buffer.append("<a href=\"fav/getAllFav.shtml?number=" + (Integer.parseInt(number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("下一页");
		} else {
			buffer.append("<a href=\"fav/getAllFav.shtml?number=" + (Integer.parseInt(number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("尾页");
		} else {
			buffer.append("<a href=\"fav/getAllFav.shtml?number=" + (maxPage - 1) + "\">尾页</a>");
		}
	...
	// 按条件查询数据 (模糊查询)
	@RequestMapping("queryFavByCond.shtml")
	...
	// 按主键查询数据
	@RequestMapping("getFavById.shtml")
