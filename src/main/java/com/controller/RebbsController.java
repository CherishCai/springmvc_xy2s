...
public class RebbsController extends BaseController {
	...
	// 准备添加数据
	@RequestMapping("createRebbs.shtml")
	...
	// 添加数据
	@RequestMapping("addRebbs.shtml")
	public String addRebbs(Rebbs rebbs) {
		rebbs.setRebbsid(VeDate.getStringDatex());
		this.rebbsDAO.insertRebbs(rebbs);
		return "redirect:/rebbs/createRebbs.shtml";
	}

	// 通过主键删除数据
	@RequestMapping("deleteRebbs.shtml")
	public String deleteRebbs(String id) {
		this.rebbsDAO.deleteRebbs(id);
		return "redirect:/rebbs/getAllRebbs.shtml";
	}

	// 更新数据
	@RequestMapping("updateRebbs.shtml")
	...
	// 显示全部数据
	@RequestMapping("getAllRebbs.shtml")
	public String getAllRebbs(String number, Map<String, Object> map) {
		...
		if ((Integer.parseInt(number) + 1) == 1) {
			...
			buffer.append("<a href=\"rebbs/getAllRebbs.shtml?number=0\">首页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if ((Integer.parseInt(number) + 1) == 1) {
			buffer.append("上一页");
		} else {
			buffer.append("<a href=\"rebbs/getAllRebbs.shtml?number=" + (Integer.parseInt(number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("下一页");
		} else {
			buffer.append("<a href=\"rebbs/getAllRebbs.shtml?number=" + (Integer.parseInt(number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (maxPage <= (Integer.parseInt(number) + 1)) {
			buffer.append("尾页");
		} else {
			buffer.append("<a href=\"rebbs/getAllRebbs.shtml?number=" + (maxPage - 1) + "\">尾页</a>");
		}
	...
	// 按条件查询数据 (模糊查询)
	@RequestMapping("queryRebbsByCond.shtml")
	...
	// 按主键查询数据
	@RequestMapping("getRebbsById.shtml")
